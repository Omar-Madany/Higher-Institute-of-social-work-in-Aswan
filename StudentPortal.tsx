        ...
        if (user) {
          ...
          try {
            ...
            if (docSnap.exists()) {
              ...
              // لا يوجد ملف تعريف في user_profiles — قبل افتراض أنه طالب،
              // نتحقق أولاً مما إذا كان هذا الحساب مسجلاً كأدمن حقيقي في
              // مجموعة admins (وهي نفس الطريقة التي تتحقق بها لوحة تحكم الإدارة).
              let isRegisteredAdmin = false;
              try {
                const adminDocRef = doc(db, "admins", user.uid);
                const adminDocSnap = await getDoc(adminDocRef);
                isRegisteredAdmin = adminDocSnap.exists();
              } catch (adminCheckErr) {
                console.warn("Could not verify admin status for fallback profile:", adminCheckErr);
              }
